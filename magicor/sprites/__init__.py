...
class AnimationGroup(pygame.sprite.Group):
    ...
    def sort(self, key=None):
        """Reinsert sprites in sorted order for Pygame 2 groups."""
        if key is None:
            key = lambda sprite: (sprite.y, sprite.x)
        ordered = sorted(self.sprites(), key=key)
        self.empty()
        self.add(*ordered)
    ...
    def _getSprites(self, x, y, width, height, exclude, type_, single = True):
        ...
        self.r.left = x
        self.r.top = y
        # Magicor uses zero-width/height rectangles as one-pixel probes.
        # Pygame 2 treats empty Rects as non-colliding, unlike the Pygame 1
        # behavior the original game relied on.
        self.r.width = max(1, width)
        self.r.height = max(1, height)
        for sprite in self.sprites():
            if (sprite != exclude
...
def collide_stick(self, bounceDir, toTravel, spriteToFollow=None):
    ...
    if "stick" in self._animations:  # lava can have more than one stick
        # sequence: born, stick to world, lose stone
