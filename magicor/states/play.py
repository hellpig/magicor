            ...
            if self.ending < 0:
                ...
                if self.ending == 0:
                    if self.player._finished:
                        ...
                        key = "time_" + self.level.title
                        if key in self.config:
                            best_time = min(self.config.getInt(key), this_time)
                        else:
                            best_time = this_time
                        print("best time for %s is %s" % (self.level.title, best_time))
                        self.config[key] = best_time
                        fn = "%s/magicor.conf" % self.config.get(
                            "user_path", "~/.magicor")
