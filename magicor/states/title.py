...
class TitleState(MenuState):
    ...
    def __init__(self, config, data, screen, startMusic = True):
        ...
        self.lights.sort(key=lambda sprite: sprite.y)
        if self.logo:
        ...
        for path, levelInfo in self.resources.loadLevelData():
            for filename, data in levelInfo:
                ...
                if not theme.startswith("_") or config.get("devmode"):
                    ...
                    self.levelPaths[level] = filename
        self.levels.sort(key=lambda level: ((level.theme or ""), level.title))
        # Start at the first level that has not yet been completed.
    ...
    def run(self):
        ...
        if self.levels:
            ...
            for i in range(self.selected - self.NUM_OPTIONS,
                ...
                if 0 <= i < len(self.levels):
                    completion_time = self.config.getInt(
                        "time_" + self.levels[i].title)
                    if completion_time:
                        minutes, seconds = divmod(completion_time, 60)
                        label = "%s (%sm%ss)" % (
                            self.levels[i].title, minutes, seconds)
                    else:
                        label = self.levels[i].title
                    self.text.draw(label, 0, y)
